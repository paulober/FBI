...
static Result httpc_read(httpc_context context, u32 *bytesRead, void *buffer, u32 size)
{
    ...
    if (R_SUCCEEDED(res = httpcGetDownloadSizeState(&context->httpc, &startPos, NULL)))
    {
        ...
        if (context->compressed)
        {
            ...
            {
                if ((context->bufferSize > 0 || R_SUCCEEDED(res = httpcReceiveDataTimeout(&context->httpc, &context->buffer[context->bufferSize], sizeof(context->buffer) - context->bufferSize, HTTP_TIMEOUT_NS)) || res == HTTPC_RESULTCODE_DOWNLOADPENDING))
                {
                    Result posRes = 0;
                    u32 currPos = 0;
    ...
    if (buf != NULL)
    {
        ...
        if (R_SUCCEEDED(res = httpc_open(&context, url, true)))
        {
            ...
            if (R_SUCCEEDED(res = httpc_get_size(context, &dlSize)))
            {
                ...
                u32 total = 0;
                u32 currSize = 0;
                while (total < dlSize && (checkRunning == NULL || R_SUCCEEDED(res = checkRunning(userData))) && R_SUCCEEDED(res = httpc_read(context, &currSize, buf, bufferSize)) && R_SUCCEEDED(res = callback(userData, buf, currSize)))
                {
                    if (progress != NULL)
                    {
