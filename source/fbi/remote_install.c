...
static Result remoteinstall_set_last_urls(const char *urls)
{
    ...
    if (R_SUCCEEDED(res = FSUSER_OpenArchive(&sdmcArchive, ARCHIVE_SDMC, fsMakePath(PATH_EMPTY, ""))))
    {
        ...
        if (urls != NULL && strlen(urls) != 0 && R_SUCCEEDED(res) && R_SUCCEEDED(res = fs_ensure_dir(sdmcArchive, "/fbi/")) && R_SUCCEEDED(res = FSUSER_OpenFile(&file, sdmcArchive, path, FS_OPEN_WRITE | FS_OPEN_CREATE, 0)))
        {
            u32 bytesWritten = 0;
